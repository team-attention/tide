...
- `src/backend/infrastructure/node/provider-bootstrap-artifacts.ts` owns provider MCP config generation for Claude and Antigravity and Codex launch options for Tide MCP.
...
- `src/backend/infrastructure/node/backend-entrypoint.ts` is the Backend entrypoint shared by Electron `MessagePort` mode and the MCP stdio mode.
