...
impl LspPort for RealLsp {
    ...
    fn is_initialized(&self) -> bool { self.manager.is_some() }
    fn did_open(&mut self, uri: &str, lang: manager::Language, text: &str) {
        if let Some(ref mut lsp) = self.manager { lsp.did_open(uri, lang, text); }
    }
    fn did_change(&mut self, uri: &str, text: &str) {
        if let Some(ref mut lsp) = self.manager { lsp.did_change(uri, text); }
    }
    fn did_save(&mut self, uri: &str) {
        if let Some(ref mut lsp) = self.manager { lsp.did_save(uri); }
    }
    fn did_close(&mut self, uri: &str) {
        if let Some(ref mut lsp) = self.manager { lsp.did_close(uri); }
    }
    fn request_completion(&mut self, uri: &str, line: u32, character: u32, trigger_kind: u32, trigger_char: Option<&str>) {
        if let Some(ref mut lsp) = self.manager { lsp.request_completion(uri, line, character, trigger_kind, trigger_char); }
    }
    fn trigger_characters(&self, lang: manager::Language) -> Vec<String> {
        self.manager.as_ref().map(|lsp| lsp.trigger_characters(lang).to_vec()).unwrap_or_default()
    }
    fn poll(&mut self) -> Option<manager::CompletionResponse> { self.manager.as_mut()?.poll() }
}
...
impl LspPort for NoopLsp {
    ...
    fn trigger_characters(&self, _lang: manager::Language) -> Vec<String> { Vec::new() }
    fn poll(&mut self) -> Option<manager::CompletionResponse> { None }
}
