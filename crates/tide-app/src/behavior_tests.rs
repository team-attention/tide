...
mod dock_behavior {
    ...
    fn app_with_terminal() -> (App, u64) {
        ...
        app.panes.insert(terminal_id, PaneKind::Launcher(terminal_id));
        app.focused = Some(terminal_id);
        app.stage_focused = Some(terminal_id);
        app.focus_area = FocusArea::Stage;
        (app, terminal_id)
    }
...
mod dock_global_behavior {
    ...
    fn dock_width_is_global_not_per_terminal() {
        ...
        let (mut app, t1, t2) = app_with_two_real_terminals();
        app.dock_width = 500.0;
        app.dock_open = true;

        // Focus t2 — dock_width should still be 500
        app.focused = Some(t2);
        app.swap_dock_state(t2);
        assert_eq!(app.dock_width, 500.0);
    ...
    fn switching_terminals_preserves_dock_width() {
        ...
        app.dock_width = 600.0;

        // Switch to t2
        app.focused = Some(t2);
        app.swap_dock_state(t2);

        assert_eq!(app.dock_width, 600.0, "dock width must persist across terminal switch");
    }
    ...
    fn pinned_panes_in_pinned_dock_layout() {
        ...
        let pinned = app.pinned_dock_layout.all_pane_ids();
    ...
    fn no_placeholder_when_pinned_panes_exist() {
        ...
        assert!(app.dock_open, "dock should stay open when pinned panes exist");
    }
    ...
    fn drag_into_pinned_group_pins_pane() {
        ...
        use crate::drag_drop::DropDestination;
    ...
    fn drag_out_of_pinned_group_unpins_pane() {
        ...
        app.handle_drop(e1, DropDestination::DockRoot(tide_core::DropZone::Right));
    ...
    fn pin_keeps_focused_on_pane() {
        // After pinning, self.focused stays on the pane (it moved to pinned_dock_layout)
        let (mut app, t1, _t2) = app_with_two_real_terminals();
        app.stage_focused = Some(t1);
        app.focused = Some(t1);
        ...
        assert_eq!(app.stage_focused, Some(t1));
    }
}
