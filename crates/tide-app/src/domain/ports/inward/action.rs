...
use tide_core::{InputEvent, SplitDirection};
...
pub(crate) trait ActionPort {
    ...
    fn split_pane_from(&mut self, source: tide_core::PaneId, direction: SplitDirection, cwd: Option<std::path::PathBuf>) -> Option<tide_core::PaneId>;
    fn cleanup_retained_context(&mut self, closed_pane_id: tide_core::PaneId);
    fn exit_app(&self);
}
