...
//
// Owns the LspManager state. Moved from BackgroundServices.

use std::path::Path;

pub(crate) trait LspPort {
    fn init(&mut self, root: &Path, waker: Option<tide_platform::WakeCallback>);
    fn is_initialized(&self) -> bool;
    fn did_open(&mut self, uri: &str, lang: tide_lsp::manager::Language, text: &str);
    ...
    fn trigger_characters(&self, lang: tide_lsp::manager::Language) -> Vec<String>;
    fn poll(&mut self) -> Option<tide_lsp::manager::CompletionResponse>;
}

// ── Real implementation (production) ──

pub(crate) struct RealLsp {
    manager: Option<tide_lsp::LspManager>,
}
...
impl LspPort for RealLsp {
    fn init(&mut self, root: &Path, waker: Option<tide_platform::WakeCallback>) {
        self.manager = Some(tide_lsp::LspManager::new(root.to_path_buf(), waker));
    }
...
impl LspPort for NoopLsp {
    fn init(&mut self, _root: &Path, _waker: Option<tide_platform::WakeCallback>) {}
    fn is_initialized(&self) -> bool { false }
    fn did_open(&mut self, _uri: &str, _lang: tide_lsp::manager::Language, _text: &str) {}
    ...
    fn trigger_characters(&self, _lang: tide_lsp::manager::Language) -> Vec<String> { Vec::new() }
    fn poll(&mut self) -> Option<tide_lsp::manager::CompletionResponse> { None }
}
