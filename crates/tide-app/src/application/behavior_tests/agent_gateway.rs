...
#[test]
fn codex_wrapper_omits_app_server_and_launches_direct_cli_only() {
    // Spec: docs/specs/codex-needs-input-attention.md
    // UC-5 BR-18: The Codex wrapper must not launch App Server remote mode.
    // UC-5 BR-19: Direct Codex launch keeps the existing hook and MCP injection.
    // Spec: docs/specs/agent-auto-integration.md
    // UC-4 BR-11: The Codex wrapper keeps an EXIT fallback for agent-detached.
    let wrapper_path = format!("{}/resources/bin/codex", env!("CARGO_MANIFEST_DIR"));
    let wrapper = std::fs::read_to_string(&wrapper_path)
        .unwrap_or_else(|err| panic!("failed to read {wrapper_path}: {err}"));
    ...
    assert!(!wrapper.contains("CODEX_APP_SERVER"));
}
