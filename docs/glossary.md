...
| **Wrapped Agent Completion Notification** | concept | A non-alert macOS notification emitted when a `Wrapped Agent` finishes a turn and provides a fresh `Notification Snippet` on an `Idle` hook payload. It is separate from `NeedsInput` attention and must not drive inactive-`Workspace` alert chrome or `RequestUserAttention`. |
...
| **AgentChromeState** | `AgentChromeState` | Renderer-facing visual state for a wrapped-agent dot: `ConnectedIdle`, `Running`, or `Attention`. `Attention` covers unresolved wrapped-agent `Idle` completion and unresolved `NeedsInput`, while `ConnectedIdle` comes from `Wrapped Agent Presence` without an active lifecycle status. `AgentChromeState` is not itself a routing state. |
...
| **Tide Instance** | concept | One running Tide app process identified by its process ID and Agent Gateway socket path (`$TMPDIR/tide-<pid>.sock`). |
| **Notification Activation Relay** | concept | A macOS notification activation handoff from a non-owning `Tide Instance` to the owning `Tide Instance` identified in the notification target. The relay must focus the owning `Workspace` and source `Pane` without surfacing a new visible Window from the non-owning instance. |
