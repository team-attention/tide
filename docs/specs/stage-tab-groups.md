...
Stage supports `LeafGroup(TabGroup)` nodes identically to Dock. Any Pane type (not just Terminal) can be grouped into a Stage TabGroup. Tab bar UI follows a unified design referencing cmux.com and VS Code patterns.

1. **Stage leaf type**: Both `Node::Leaf(PaneId)` and `Node::LeafGroup(TabGroup)` in Stage's SplitLayout.
2. **Tab bar rendering**: Per-TabGroup tab bar shown for LeafGroup nodes with 2+ tabs. Single-tab LeafGroups render as normal pane headers (no tab bar). Uses a unified tab bar component shared with Dock, styled after cmux/VS Code (compact, horizontally scrollable, close button per tab, active tab highlight).
...
5. Tab bar style follows cmux/VS Code design: compact height, horizontal layout, active tab distinguished by background color, inactive tabs subtly dimmed.
