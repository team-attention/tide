...
2. **No fuzzy picker.** The only way to reach a Workspace is sequential Cmd+[/] cycling or clicking the sidebar. With 5+ Workspaces this is slow. Every researched tool (Zellij, tmux-sessionizer, VS Code Ctrl+R, JetBrains project widget) provides a single-keystroke fuzzy picker.

3. **No visual identity.** All Workspaces look identical. The titlebar shows "Tide" or "Tide . N" (see `adapter/outward/view/chrome/titlebar.rs` line 28-29). No color coding, no branch name, no persistent indicator of which Workspace is active beyond the sidebar.
...
5. **Sidebar shows CWD only for active Workspace.** The titlebar view (line 344-370) renders CWD text only when `is_active` is true. Inactive Workspaces show just a name. No branch, no Pane count, no status indicators beyond the agent notification dot.
...
3. **Color-as-identity** — Each Workspace gets an accent color from a fixed palette (8-10 colors, assigned round-robin by creation order). The color appears as: (a) a dot/stripe in the sidebar item, (b) the title bar Workspace indicator, and (c) the focused Pane border tint. Inspired by VS Code Peacock, but built-in and automatic.
...
5. **Richer Sidebar** — All Workspace sidebar items show: name (with color dot), git branch (if detectable), and Pane count. CWD is shown for all Workspaces (read from cold-stored terminal contexts), not just the active one.
...
- Update `render_titlebar_and_sidebar` to display branch + count for all items.
...
- Render a color dot in sidebar items and the titlebar indicator.
...
- Add `GlobalAction::OpenWorkspacePicker` with keybinding (Cmd+Shift+P or similar; exact key TBD with user).
...
| `adapter/outward/view/chrome/titlebar.rs` | Richer sidebar rendering, persistent indicator |
...
- **Actor**: User
- **Trigger**: GlobalAction::OpenWorkspacePicker (keybinding TBD)
...
- **Actor**: User
- **Trigger**: Double-click Workspace name in sidebar, or future rename action
- **Precondition**: Target Workspace exists
- **Flow**:
  1. Show inline rename input in the sidebar item
...
- **Flow**:
  ...
  3. Render color dot in sidebar, titlebar, and Pane border tint
...
- **Business Rules**:
  ...
  - BR-14: Color dot is always visible in the titlebar indicator, regardless of sidebar state
...
### UC-7: RenderRicherSidebar

- **Actor**: System (view layer)
- **Trigger**: Sidebar is visible and needs rendering
...
- **Postcondition**: All Workspace sidebar items show rich metadata
- **Business Rules**:
...
| UC-7: RenderRicherSidebar | BR-18 | `sidebar_shows_branch_for_inactive_workspaces()` |
| UC-7: RenderRicherSidebar | BR-19 | `sidebar_shows_pane_count_for_all_workspaces()` |
| UC-7: RenderRicherSidebar | BR-20 | `sidebar_renders_color_dot_for_each_workspace()` |
...
| View - Chrome | `adapter/outward/view/chrome/titlebar.rs` | Richer sidebar, persistent indicator |
