...
| Foreground + different pane focused | Tab dot (orange) + blink | Tab dot (dim green, static) |
| Different workspace | Workspace sidebar dot | Workspace sidebar dot |
...
- **Flow**:
  ...
  4. Request redraw each frame (to sustain the blink animation)
- **Postcondition**: Orange dot pulses smoothly
- **Business Rules**:
...
- **Flow**:
  ...
  2. If `true`, render a small dot to the right of the workspace name
  3. Dot color: orange (same as NeedsInput — use highest urgency at workspace level)
- **Postcondition**: User can identify workspaces with pending notifications in the sidebar
- **Business Rules**:
