...
- **Flow**:
  ...
  3. The Codex wrapper configures a `Stop` hook to forward hook stdin JSON into Tide's Codex-specific classifier
...
- **Business Rules**:
  ...
  - BR-9: The Codex wrapper forwards the documented Codex `Stop` hook payload through the checked-in hook config
