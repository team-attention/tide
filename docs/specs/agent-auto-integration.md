...
- The checked-in Claude and Gemini wrappers already map their documented hook names into `tide notify` lifecycle events and forward hook `stdin` JSON for `Notification` and `Stop` or `AfterAgent`.
- The checked-in Codex wrapper uses the documented Codex hook contract by wiring `UserPromptSubmit` and `Stop` into `tide notify`, and keeps a shell `EXIT` fallback lifecycle path.
- The checked-in Codex-specific spec already requires documented hook-driven lifecycle reports so each new Codex turn can return the source `Pane` to `Running`, while wrapper launch only marks `Wrapped Agent Presence`.
- The official Codex hooks documentation exposes `UserPromptSubmit` and `Stop` as turn-scoped hooks and documents `last_assistant_message` on the `Stop` payload.

### To-Be

- Keep auto-integration zero-config for the fixed Wrapped Agent set: `claude`, `codex`, and `gemini`.
- Keep the Codex wrapper aligned with the documented Codex hook contract by using `UserPromptSubmit` for turn start and `Stop` for turn completion.
...
- Forward documented hook `stdin` JSON from the checked-in Claude, Codex, and Gemini wrappers so Tide can derive a `Notification Snippet` from official hook payload fields instead of guessing from shell output alone.
...
4. Use the documented Codex `Stop` hook to forward completion payloads into Tide without relying on the legacy `notify` completion path.
...
| `codex` | launch-time `agent-attached`, `UserPromptSubmit` -> `agent-running`, `Stop` -> `agent-idle` with forwarded hook `stdin` JSON, `EXIT` -> `agent-detached`, plus OSC 9 fallback `tide:wrapped-agent:codex:<event>` | launch -> presence only, `UserPromptSubmit` -> `Running`, `Stop` -> `Idle` plus optional `Wrapped Agent Completion Notification`, `EXIT` -> clear presence | `crates/tide-app/resources/bin/codex`, `crates/tide-app/src/adapter/inward/cli_adapter/notify.rs::run_notify`, `crates/tide-app/src/adapter/inward/cli_adapter/commands.rs::cli_notify`, `crates/tide-app/src/app.rs::handle_terminal_notification`, `crates/tide-app/src/app.rs::route_agent_notification` |
...
- **Flow**:
  ...
  3. The Codex wrapper configures `UserPromptSubmit` and `Stop` hooks to forward lifecycle payloads through `tide notify`
  4. The Codex wrapper keeps an `EXIT` fallback for `agent-detached`
  5. Tide does not assume an unsupported Codex `NeedsInput` signal beyond the documented hook surface
...
- **Business Rules**:
  ...
  - BR-9: The Codex wrapper forwards the official Codex `Stop` hook payload through `tide notify --payload-stdin`
  - BR-10: The Codex wrapper keeps an `EXIT` fallback for `agent-detached`
  - BR-11: The Codex wrapper does not claim a Codex `NeedsInput` signal without a documented Codex hook for it
  - BR-12: Codex completion parsing belongs to the Tide-side helper documented in `docs/specs/codex-needs-input-attention.md`, not to the wrapper script

### UC-5: ForwardStructuredHookPayloads

- **Actor**: Tide wrapper maintainer
- **Trigger**: A checked-in Claude, Codex, or Gemini wrapper hook needs Tide-visible response text
...
- **Business Rules**:
  ...
  - BR-14: Codex `Stop` hook commands forward their hook `stdin` JSON through `tide notify --payload-stdin`
  - BR-15: Gemini `Notification` and `AfterAgent` hook commands forward their hook `stdin` JSON through `tide notify --payload-stdin`
  - BR-16: `tide notify --payload-stdin` must be additive to the existing explicit event and `PaneId` arguments
...
| UC | BR | Test function |
|----|-----|---------------|
| UC-4 | BR-7 | `codex_wrapper_uses_the_explicit_hook_contract` |
| UC-4 | BR-8 | `codex_wrapper_uses_the_explicit_hook_contract` |
| UC-4 | BR-9 | `codex_wrapper_uses_stop_hook_for_completion` |
| UC-4 | BR-10 | `codex_wrapper_uses_the_explicit_hook_contract` |
| UC-4 | BR-11 | `codex_stop_hook_payload_never_maps_to_needs_input` |
| UC-4 | BR-12 | `codex_stop_hook_completion_notification_uses_last_assistant_message_snippet` |
...
| UC-5 | BR-14 | `codex_wrapper_uses_stop_hook_for_completion` |
| UC-5 | BR-15 | `gemini_wrapper_forwards_hook_stdin_payloads_for_notification_and_after_agent` |
...
| Wrapper | `crates/tide-app/resources/bin/codex` | Defines the evidence-backed Codex hook contract |
