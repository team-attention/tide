...
| **Wrapped Agent Provider** | concept | A coding-agent CLI that Tide ships a checked-in `Agent Wrapper` for. The fixed set is `claude` (Claude Code), `codex`, `agy` (Antigravity), and `opencode`. Runtime names stay provider-neutral; each provider is just another `Wrapped Agent` speaking the `Tide MCP Runtime` contract. |
