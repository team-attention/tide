...
| **Editor Pane** | `PaneKind::Editor` | `domain/pane/editor.rs` | A Pane that wraps `EditorState` with mode state, Soft Wrap, selection, search, preview, LivePreviewMode, and Editor Chrome behavior. |
...
| **Browser Pane Screenshot** | `BrowserPaneScreenshot` | `domain/pane/browser.rs` | A captured raster image of a Browser Pane's rendered `WKWebView` page (PNG bytes + pixel `width`/`height` + `device_scale`), surfaced to a Wrapped Agent as an MCP image content block so the agent can see the page as pixels, not only as BrowserSnapshot text and Browser Page Map structure. Mirrors the v2 Desktop computer-use screenshot. |
...
| **Selection** | `Selection` | `domain/pane/mod.rs` | A Pane-local text range with an anchor row/column and active end row/column. Terminal, Editor, and Diff Panes use it for text selection, copy, and Context Artifact capture. |
...
| **Markdown Reading/Source modes** | concept | A Markdown Pane has exactly two modes: **Reading** (read-only `render_markdown_preview` formatted view, the open default) and **Source** (Plain raw markdown + syntax highlighting). The Editor Chrome badge toggles between them (`edit`/`read`). LivePreviewMode (a former third inline-WYSIWYG mode) was removed; see `docs/specs/markdown-reading-edit-modes.md`. |
...
| **Markdown List Continuation** | `MarkdownListContinuation` | `domain/pane/editor.rs` | The Markdown Pane authoring rule that derives the next list marker, or exits an empty list item, when Enter is pressed at the end of a list line. |
...
| **Browser Auth Popup** | concept | A sign-in popup launched from Browser Pane content that depends on opener or channel state to complete a callback into the originating page. Supported Browser Auth Popup flows use a native popup `WKWebView` returned from `WKUIDelegate::createWebView` so WebKit preserves opener relationship. |
| **Browser Content Zoom** | concept | Per-Browser Pane page content scale applied to the native `WKWebView`. `Cmd+Plus`, `Cmd+Minus`, `Cmd+0`, and native pinch gestures adjust Browser Pane content without changing Tide's app font size. |
| **Tide MCP Runtime** | concept | The provider-neutral Agent Gateway MCP contract exposed to Wrapped Agents and external MCP clients. It observes and mutates Tide Workspaces, Panes, and layout targets without depending on provider-specific browser tools. |
...
| **Wrapped Agent Provider** | concept | A coding-agent CLI that Tide ships a checked-in `Agent Wrapper` for. The fixed set is `claude` (Claude Code), `codex`, `agy` (Antigravity), `opencode`, and `vibe` (Mistral Vibe). Runtime names stay provider-neutral; each provider is just another `Wrapped Agent` speaking the `Tide MCP Runtime` contract. |
