...
| **Fixed-Width Live Preview Block** | concept | A LivePreviewMode block element, such as a code block or source-valid Markdown table, that preserves intrinsic source width, renders each source row as one display row under Soft Wrap, and owns block-scoped horizontal scroll. |
