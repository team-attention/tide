...
Focused `Pane`s stay easy to identify in both Stage and Dock through the current brighter active header/tab treatment, without adding a new full-pane outline around the terminal body. `Terminal-Owned Attention` is dot-only: it renders only on `Workspace` list items and on direct wrapped-agent owner `Terminal`s in Stage. A direct wrapped-agent Stage `Terminal` with `Running` shows a solid green dot. A direct wrapped-agent Stage `Terminal` with unresolved `Idle` or `NeedsInput` shows the same solid orange dot family. A direct wrapped-agent Stage `Terminal` with `Wrapped Agent Presence` but no active `AgentStatus` shows a solid idle-presence dot in a muted cool color. Tide does not add wrapped-agent fill or underline to the entire Stage `Pane` surface. Non-terminal `Pane`s never inherit wrapped-agent dots through `Associated Terminal`, and Dock chrome never renders a wrapped-agent dot. If an alerting Stage `Terminal` is clipped out of the `ViewMode::Stacked` flat Stage tab strip, the tab-strip edge in the hidden direction shows the same solid orange dot. Workspace items show the strongest direct Stage-terminal state for that `Workspace`, regardless of whether the `Workspace` is active or inactive: solid orange alert takes precedence over green running, which takes precedence over idle-presence. Wrapped-agent lifecycle transitions invalidate chrome and request one redraw, but unresolved attention does not keep the render loop active. Active headers, active stacked Stage tabs, and busy `Terminal Pane` headers preserve readable labels before optional git badges consume the remaining width. The macOS titlebar remains 32 logical pixels high, while Pane headers, shared tab bars, and the FileTree View header share a denser 26 logical-pixel height and 13 logical-pixel type. Header typography and icon layout use fixed chrome metrics so content font zoom does not enlarge the surrounding chrome. A Dock `TabGroup` with one `Pane` uses the single-`Pane` header chrome instead of a shared tab bar, so the title and close affordance sit in the same layout family as the rest of Tide. A Stacked Terminal Context Surface with one `Pane` follows the same rule so the active file or surface name remains visible. Stacked Terminal Context Surface headers omit the redundant identity badge entirely so Pane tabs receive that width; Split mode retains only `Context: <owner>`. The shared active-tab width budget continues to reserve space for the Stage-terminal dot, shared tab scrolling remains stable and directionally consistent, and git badges continue to refresh on a near-immediate frame-scale delay. When repo-to-repo navigation queues multiple git refreshes, the background poller must prefer the latest pending `cwd` request instead of spending extra time publishing obsolete repo results first.
...
5. Map direct Stage-terminal `Idle` and `NeedsInput` to the same solid orange alert dot family.
6. Map `Wrapped Agent Presence` plus `AgentStatus = None` to a solid idle-presence dot on direct Stage `Terminal`s.
7. Project Workspace list-item dots from Stage terminals only: solid orange for alert, green for running, idle-presence color for connected idle, using the same strongest-state precedence for active and inactive `Workspace`s.
...
9. When an alerting Stage-terminal tab is scrolled outside the visible shared-tab range, render the solid orange indicator on the corresponding tab-strip edge.
10. Request redraw when wrapped-agent lifecycle state changes, but do not request continuous redraw while attention remains unresolved.
11. Reserve a minimum title region in active headers and active tabs, eliding optional git badges before the title disappears.
12. Use a readable shared label color path for busy `Terminal Pane` headers so terminal names do not fall back to a dimmed badge color.
13. Keep the macOS titlebar at 32 logical pixels with its 14 logical-pixel reference control geometry while Pane headers, shared tab bars, and the FileTree View header use one exact 26 logical-pixel height and 13 logical-pixel type; keep header typography and icon layout fixed when content font zoom changes.
14. Reserve shared-tab width for the Stage-terminal dot whenever a visible Stage `Terminal` has direct wrapper-managed lifecycle state.
15. Keep explicit horizontal tab scrolling stable by auto-fitting the active tab only on active-tab changes, not on every render while the user is manually browsing overflowed tabs.
16. Make shared tab bars treat horizontal precise delta as the primary gesture signal, leaving vertical fallback for wheel-style scrolling when no horizontal delta is present.
17. Give shared tab bars a modest starter step only on a fresh precise stroke or an immediate direction change, then fall back to lighter continuous motion so tab scrolling feels responsive without becoming over-sensitive.
18. Use the same horizontal delta sign convention for shared tab bars that editor and diff panes already use, so horizontal scroll direction stays consistent across Tide.
19. Clamp stored shared-tab scroll offset to the visible bounds at input time, not only during render, so edge overscroll never creates hidden reverse-scroll debt.
20. Schedule PTY-driven terminal badge refresh on a near-immediate frame-scale delay instead of a long fixed delay.
21. Only consume header-area scroll for shared tab bars that actually have horizontal overflow; otherwise route the scroll to the pane content below.
22. Clear stale terminal git and worktree badge state as soon as cached `cwd` changes, before the next background git poll result arrives.
23. Collapse queued git-poll `cwd` refresh requests to the latest pending request before publishing results, so repo chrome repopulates promptly after repo-to-repo navigation.
24. Render Dock shared tab bar chrome only when the Dock `TabGroup` or Stacked Terminal Context Surface actually has multiple tabs; a single Dock `Pane` falls back to the normal single-`Pane` header.
25. Omit the Terminal Context Surface identity badge in Stacked mode; in Split mode render only the compacted owning Stage `Terminal` label as `Context: <owner>`.
...
- **Business Rules**:
  ...
  - BR-4: `Idle` and `NeedsInput` share the same solid orange Stage-terminal alert dot family
...
- **Flow**:
  ...
  3. Tide renders the Stage-terminal dot with the color for that state
...
- **Business Rules**:
  ...
  - BR-10: Unresolved `Idle` and `NeedsInput` render the same solid orange Stage-terminal dot and do not require continuous redraw
...
- **Business Rules**:
  - BR-11: A `Workspace` item with unresolved Stage-terminal `Idle` or `NeedsInput` renders a solid orange dot
...
- **Flow**:
  ...
  3. Tide renders the solid orange edge indicator on the corresponding tab-strip edge
- **Postcondition**: Hidden wrapped-agent alert tabs remain discoverable while scrolled out of view
- **Business Rules**:
  - BR-14: A hidden alert tab left of the visible range renders a solid orange left-edge indicator
  - BR-15: A hidden alert tab right of the visible range renders a solid orange right-edge indicator
...
- **Flow**:
  1. Tide keeps the macOS titlebar at 32 logical pixels and applies the shared 26 logical-pixel height to Pane headers, shared tab bars, and the FileTree View header
  2. Tide uses fixed 13 logical-pixel type and fixed chrome metrics for header typography and icons, independent of content font zoom, and vertically centers them within each band without shrinking the native titlebar controls
...
- **Postcondition**: The titlebar and content header bands use their intended compact heights, focused tabs feel emphasized, and terminal labels remain readable
- **Business Rules**:
  - BR-19: The macOS titlebar is exactly 32 logical pixels high and retains its 14 logical-pixel reference control geometry; Pane headers, shared tab bars, and the FileTree View header are exactly 26 logical pixels high and use 13 logical-pixel type; header typography and icon layout remain fixed when content font zoom changes
...
6. The macOS titlebar remains exactly 32 logical pixels high with its 14 logical-pixel reference control geometry, while Pane headers, shared tab bars, and the FileTree View header share an exact 26 logical-pixel height, 13 logical-pixel type, and fixed chrome metrics.
7. Wrapped-agent lifecycle transitions invalidate chrome and request redraw, while unresolved attention does not request continuous redraw.
...
| UC-3 | BR-10 | `attention_stage_terminal_renders_a_solid_orange_dot` |
| UC-3 | BR-10 | `wrapped_agent_attention_color_is_time_independent` |
| UC-3 | BR-10 | `unresolved_wrapped_agent_attention_does_not_schedule_animation_redraw` |
| UC-4 | BR-11 | `inactive_workspace_alert_renders_a_solid_orange_dot` |
| UC-4 | BR-11 | `active_workspace_alert_renders_a_solid_orange_dot` |
...
| UC-7 | BR-19 | `titlebar_and_content_headers_use_distinct_compact_heights` |
| UC-7 | BR-19 | `header_chrome_metrics_do_not_follow_content_font_zoom` |
...
| Event loop adapter | `crates/tide-app/src/adapter/inward/event_loop_adapter/mod.rs` | Transition-driven redraw without unresolved-attention animation frames |
