...
- **Flow**:
  1. Check modal interception chain (config_page → context_menu → file_tree_rename → git_switcher → file_finder → save_as_input → context_comment_composer → branch_cleanup → save_confirm → CompletionPopup)
  2. If modal consumes event → RETURN immediately
  3. Check FocusArea:
     - FileTree → handle FileTree View keys (arrows, enter, etc.)
...
- **Business Rules**:
  ...
  - BR-9a: `Cmd+Shift+O` resolves to `FileFinder`
...
- **Flow**:
  1. Determine TextInputTarget via priority chain:
     ...
     - ContextMenu or SaveConfirm open → Consumed
     - FileTreeRename open → FileTreeRename
     - GitSwitcher open → GitSwitcher
     - FileFinder open → FileFinder
     - SaveAsInput open → SaveAsInput
     - ContextCommentComposer open → ContextCommentComposer
     - SearchBar focused → SearchBar(id)
     - Browser URL bar focused → BrowserUrlBar(id)
     - Focused Browser Pane without URL bar focus → Consumed
...
- **Business Rules**:
  ...
  - BR-18a: Clipboard paste failure is consumed without mutating the focused Pane
...
- **Flow**:
  ...
  2. FileTree View toggle: show/hide FileTree View while preserving the current FocusArea on open
  3. Stacked toggle: set/clear stacked state for the current FocusArea, or target Dock explicitly through DockToggleStacked
- **Postcondition**: Focus, stacked mode, or FileTree View state updated
- **Business Rules**:
  ...
  - BR-24: FileTree View toggle shows/hides FileTree View; opening preserves the current FocusArea, and closing falls back to Stage only when FileTree owns focus
...
- **Business Rules**:
  ...
  - BR-29: NewTab creates a right-side Stage Terminal Pane through a vertical split when FocusArea is Stage, or a Launcher split in the Terminal Context Surface when FocusArea is Dock
...
| UC-1 | BR-9a | `domain/input/tests.rs` | `meta_shift_o_triggers_file_finder` |
...
| UC-2 | BR-18a | `text_input_routing` | `paste_action_consumes_clipboard_failure_without_mutating_editor` |
...
| UC-3 | BR-24 | `global_actions` | `toggle_file_tree_from_stage_opens_without_moving_focus` |
...
| UC-4 | BR-29 | `global_actions` | `new_tab_global_action_creates_terminal_pane_in_stage` |
