...
- Codex plugins, Claude plugins, Antigravity plugins, legacy Gemini imports, and MCP servers are not shown as one generic plugin system.
...
| Agent Binding | Codex CLI, Claude Code, or Antigravity CLI. |
...
| Antigravity CLI | `--print` is non-interactive. Interactive flags include `--prompt-interactive`, `--continue`, and `--conversation`. It also exposes `--log-file`, `--sandbox`, and `--dangerously-skip-permissions`. Local state includes a cwd-to-conversation cache, opaque `.pb` conversation records, and hook-provided readable transcript JSONL under `~/.gemini/antigravity-cli/brain/<conversation-id>/.system_generated/logs/transcript.jsonl`. A local hook smoke observed `conversationId`, `transcriptPath`, `artifactDirectoryPath`, `PreInvocation`, `PostInvocation`, and `Stop` with `fullyIdle`. No local help evidence showed a stream-json or app-server-style live API. | Antigravity Integration uses hidden PTY as its runtime transport. Hooks, explicit logs, transcript paths, and conversation refs are Provider Signals. The Integration should bootstrap the provider hook config before launching the hidden PTY so it can capture the readable Raw Agent Session ref. |
...
- Antigravity first-run onboarding and cwd trust were observed in local smoke. The same readiness model should be assumed for Codex and Claude until clean-directory trust behavior is separately proven.
