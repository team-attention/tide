...
not a hardcoded list. Current file-based discovery covers Claude Code commands
and Codex skills.
...
- **Trigger prefixes**: `/` lists provider commands; `$` lists skills. Current
  filesystem-backed entries are Claude slash commands and Codex skills. A provider
  with no entries for a prefix shows nothing for it.
...
- Discovery must be explicit per provider; unsupported providers return no
  file-discovered rows instead of guessing filesystem conventions.
