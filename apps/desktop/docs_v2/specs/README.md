...
| [Backend-Owned Process Lifecycle](backend-owned-process-lifecycle.md) | Drafted | One registered spawn/stop/crash lifecycle for Codex, Claude, opencode, provider helpers, LSPs, and terminals, with vendor-native protocol cleanup retained in adapters. |
...
| [Provider Catalog Ownership And Model Selection](provider-catalog-ownership-and-model-selection.md) | Drafted | Separate selected Thread launch values from provider-owned available options; removes catalog ownership from thread.list/module globals. |
