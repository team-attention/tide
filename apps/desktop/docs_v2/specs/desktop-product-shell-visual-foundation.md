...
- Left Rail as compact work history.
...
- Figma node `1303:55` shows layout variants where Left Rail, Agent Chat, Workbench, and FileTree each own a 52px top row; it does not use a separate global title bar.
- Figma node `1303:55` shows Left Rail Thread Rows without visible leading Thread icons, Project Rows with folder icons, Project hover actions as more and new-thread icons, and archive confirmation as a transient inline row action.
- Figma node `1288:2` defines Left Rail row states: Thread Row has no leading icon; Thread hover exposes pin/archive; archive click swaps the action slot into a single Confirm pill until pointer leave; Project open/closed state is expressed by folder icon; Project hover exposes more and new-thread actions; context menus keep the owning row highlighted.
...
```text
Left Rail | Agent Chat | optional Workbench
```

Left Rail Thread Rows are compact navigation rows, not boxed cards.
...
- Left Rail owns window traffic controls and the Left Rail collapse action while it is open.
...
### D10. Left Rail row menus are row-owned transient state

Thread Row and Project Row context menus belong to their owning Left Rail row.
...
Agent Icon is rendered by Agent identity where identity is ambiguous, but canonical Left Rail Thread Rows do not show a default leading Thread icon.

## Contracts

No Shared Contract changes are required for the Product Shell visual layer.

This slice consumes existing Desktop view models and fixture Left Rail data. Composer submit must leave the renderer through BackendCommand drafts rather than creating fake Agent Session output.
...
3. Left Rail shows work history affordances.
...
1. Left Rail shows Pinned, Projects, and Scratch.
...
### UC-5: User opens an existing Thread from Left Rail
...
2. Product Shell clears the active Thread without deleting the Left Rail history.
...
3. Left Rail is work history, not an IDE FileTree.
...
10. Left Rail actions and Composer controls that appear actionable update Product Shell state.
...
| Left Rail uses work history | `left_ui_renders_project_grouped_thread_rows_without_thread_icons` renders Pinned, Projects, Scratch, Project folder rows, and iconless Thread Rows. |
