...
| UC-2 BR-3: Product Shell selections reach Backend runtime unchanged | `product_shell_thread_start_command_reaches_backend_with_selected_agent_binding` submits a Product Shell provider-CLI start command through the Backend Contract adapter and verifies Provider Readiness, Agent Runtime start, and terminal input all use the selected Agent Binding and Launch Options. |
