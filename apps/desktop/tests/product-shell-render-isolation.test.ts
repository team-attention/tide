...
import { Profiler, act, createElement, memo } from "react";
...
const Rail = memo(function Rail() {
  ...
  return createElement("div", { "data-testid": "rail" }, searchQuery);
});
...
const Workbench = memo(function Workbench() {
  ...
  return createElement("div", { "data-testid": "workbench" }, String(open));
});
  ...
  await act(async () => {
    root.render(
      createElement(
        ...
        createElement(
          Profiler,
          { id: "workbench", onRender: () => { commits += 1; } },
          createElement(WorkbenchColumnView, { handlers }),
        ),
      ),
    );
  });
