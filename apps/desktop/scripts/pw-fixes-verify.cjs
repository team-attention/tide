...
(async () => {
  ...
  const addVendor = page.locator(".chip-popover__row, .choice-surface__row, [role='option'], button").filter({ hasText: /add a vendor/i }).first();
  ...
  const reconnectTiles = await page.locator('.oc-tile[data-reconnect="true"]').allInnerTexts();
  ...
  await page.waitForTimeout(300);
  const nameInput = page.locator(".workbench-launcher-new-file-input");
  ...
  const editorPanes = await page.locator(".workbench-column, [data-column='workbench'], .cm-editor").count();
