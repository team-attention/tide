...
//
//   --behavioral   (permission only; claude/gemini/codex)
...
//     --agent <claude|codex|gemini|opencode> [--option permission|model|reasoning]
...
if (!["claude", "codex", "gemini", "opencode"].includes(agent)) {
  console.error("Usage: --agent <claude|codex|gemini|opencode> [--option permission|model|reasoning] [--behavioral]");
  process.exit(2);
}
...
const EXPECTED = {
  ...
  gemini: { permission: "live", model: "next_turn", reasoning: "next_turn" },
...
const TARGET = {
  ...
  gemini: { permission: "yolo", model: "gemini-2.5-flash", reasoning: "high" },
