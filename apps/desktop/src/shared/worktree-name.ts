...
//
// A worktree's branch + directory share one name. It must be safe ASCII
// (`[a-z0-9-]`), so it is derived deterministically and never carries non-ASCII
// (e.g. a Korean first message falls through to a hash). Process-agnostic.
...
// Resolve the final worktree name from the fixed priority:
...
export function resolveWorktreeName(input: {
  ...
  const typed = slugFromMessage(input.typedName ?? "");
  if (typed.length > 0) {
    return typed;
  }
