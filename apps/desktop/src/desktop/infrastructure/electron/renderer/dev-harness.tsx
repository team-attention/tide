    ...
    payload: {
      ...
      blocks: [
        ...
        block({
          ...
          role: "tool",
          title: "Read",
          body: JSON.stringify({ file_path: "/Users/you/Workspace/tide/src/desktop/adapters/inbound/react-renderer/agent-chat/agent-chat.tsx" }),
          updatedAt: at(3),
        }),
        block({
          blockId: "r-tool-grep",
        ...
        block({
          blockId: "r-user-attach",
          kind: "message",
          role: "user",
          body: [
            "**↳ agent-chat-shell.ts L817-871**",
            "이 부분 line-height가 좀 빡빡한데 1.6으로?",
            "",
            "```ts",
            "const role = block.role === \"user\" ? \"user\" : \"agent\";",
            "```",
...
if (root) {
  try {
    ...
    createRoot(root).render(<TideProductShell initialState={state} />);
