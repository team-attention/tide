  ...
  return (
    ...
    >
      <div className="quick-open">
        <div className="quick-open__field">
          <Search size={15} strokeWidth={1.9} className="quick-open__icon" aria-hidden />
          <input
            ref={inputRef}
          ...
          />
          <span className="quick-open__count">{results.length > 0 ? `${results.length}` : ""}</span>
        </div>
        <div className="quick-open__results" ref={listRef}>
          {results.length === 0 ? (
            ...
            results.map((file, index) => {
              ...
              return (
                ...
                >
                  <span className="quick-open__row-icon" aria-hidden>
                    <Icon size={14} strokeWidth={1.7} />
                  </span>
                  <span className="quick-open__row-name">{file.name}</span>
                  {dir ? <span className="quick-open__row-dir">{dir}</span> : null}
                </button>
              );
            })
          )}
