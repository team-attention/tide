...
export function WorkbenchDiffPane(props: {
  ...
  return (
    <div className="workbench-pane-content workbench-pane-content--diff">
...
function createDiffView(diffText: string): ReactElement {
  ...
  return (
    <div className="workbench-diff" aria-label="Diff view" role="group">
      <div className="workbench-diff__stat">
        {adds > 0 ? <span className="workbench-diff__stat-add">{`+${adds}`}</span> : null}
        {dels > 0 ? <span className="workbench-diff__stat-del">{`−${dels}`}</span> : null}
        <span className="workbench-diff__stat-label">
          {adds + dels === 1 ? "1 change" : `${adds + dels} changes`}
        </span>
      </div>
      <div className="workbench-diff__body">
        {rows.map((row, index) => {
          if (row.kind === "hunk") {
            return (
              <div key={index} className="workbench-diff-row workbench-diff-row--hunk">
                <span className="workbench-diff-row__text">{row.text}</span>
              </div>
            );
          }
          const marker = row.kind === "added" ? "+" : row.kind === "removed" ? "−" : "";
          return (
            <div key={index} className={`workbench-diff-row workbench-diff-row--${row.kind}`}>
              <span className="workbench-diff-row__gutter" aria-hidden="true">{row.oldNo ?? ""}</span>
              <span className="workbench-diff-row__gutter" aria-hidden="true">{row.newNo ?? ""}</span>
              <span className="workbench-diff-row__marker" aria-hidden="true">{marker}</span>
              <span
